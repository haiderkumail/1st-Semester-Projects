...
void instructions()
{
	...
	cout << "Instructions";
	cout << "\n----------------";
	cout << "\n Press spacebar to make bullets";
	cout << "\n Press A to move left";
	cout << "\n Press D to move right";
	cout << "\n\nPress any key to go back to menu";
	getch();
}
string getField(string temp, int field)
{
	string item;
	int commaCount = 1;
...
void storeGame(int birdPos, int score)
{
	fstream file;
	file.open("game.txt", ios::out);
	file << score << "," << birdPos << endl;
	file.close();
}
void loadGame()
{
	...
	while (file.eof())
	{
		score = stoi(getField(temp, 1));
		birdPos = stoi(getField(temp, 2));
	}
	file.close();
}
